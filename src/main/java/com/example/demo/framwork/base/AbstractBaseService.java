...
import com.example.demo.framwork.utils.db.ResultMappingUtils;
...
public abstract class AbstractBaseService<T> extends AbstractPropertyFilterService<T> {
    ...
    public Page<T> findPageByPropertyFilters(Page<T> page, List<PropertyFilter> filters, String[] fieldArr) {
        ...
        if (ArrayUtils.isNotEmpty(fieldArr)) {
            ...
            for (String field : fieldArr) {
                if (StringUtils.isNotBlank(field)) {
                    field = NameUtils.covertToDbName(field);
                    if (isFirst) {
                        sql.append(field);
                        isFirst = false;
                    } else {
        ...
        String tableName = NameUtils.covertToDbName(entityClass.getSimpleName());
        sql.append(" FROM ").append(tableName).append(PropertyFilter.buildQueryStringByPropertyFilter(filters));
        ...
        if (page != null && page.isOrderBySetted()) {
            ...
            for (int i = 0; i < orderByArray.length; i++) {
                ...
                String orderName = NameUtils.covertToDbName(orderByArray[i]);
        ...
        List<T> list = selectListBySql(sqlStr);
        if (page == null) {
            page = new Page<>();
        }
        page.setResult(list);
    ...
    private List<T> selectListBySql(String sql) {
        List<Map<String, Object>> mapList = commonMapper.executeSelectSqlResultMap(sql.toString());
        return (List<T>) ResultMappingUtils.mapping(mapList, entityClass);
    }
